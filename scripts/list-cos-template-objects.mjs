...
async function listPage(config, marker = '') {
  ...
  const response = await fetch(`https://${config.host}/?${queryString}`, {
    headers: {Authorization: authorization({...config, query})}
  });
  const xml = await response.text();
  if (!response.ok)
    throw new Error(
      `COS 列表读取失败 (${response.status})：${xmlValue(xml, 'Message') || xmlValue(xml, 'Code') || response.statusText}`
    );
  ...
  return {
    objects,
    truncated: xmlValue(xml, 'IsTruncated').toLowerCase() === 'true',
    nextMarker: xmlValue(xml, 'NextMarker') || objects.at(-1)?.key || ''
  };
}
...
const base = (process.env.TENCENT_COS_BASE_PATH ?? process.env.COS_PREFIX ?? 'uploads/').replace(
  /^\/+|\/+$/g,
  ''
);
const environment = (
  process.env.TENCENT_COS_ENV_PREFIX ??
  process.env.COS_ENV_PREFIX ??
  'prod'
).replace(/^\/+|\/+$/g, '');
const project = (
  process.env.TENCENT_COS_PROJECT_PREFIX ??
  process.env.COS_PROJECT_PREFIX ??
  'calendar'
).replace(/^\/+|\/+$/g, '');
...
const templates = [...groups]
  .map(([id, files]) => ({id, files: files.sort((a, b) => a.key.localeCompare(b.key))}))
...
for (const template of templates) {
  ...
  for (const file of template.files)
    console.log(`  ${file.fileName}  ${file.size} bytes  ${file.lastModified}`);
}
