...
export default function AccountManagement() {
  ...
  const removeOrganization = async (organization: Organization) => {
    ...
    const response = await fetch(`/api/organizations?id=${encodeURIComponent(organization.id)}`, {
      method: 'DELETE'
    });
    const result = (await response.json()) as {error?: string};
    if (!response.ok) {
  ...
  return (
    <main className="account-page with-sidebar">
      <header className="system-bar page-topbar">
        <div className="page-heading">
          <b>账号、组织与权限</b>
          <span>组织内共享模板，超级管理员可访问全部组织</span>
        </div>
      </header>
        ...
        <section className="account-create">
          ...
          <form onSubmit={create}>
            ...
            <fieldset>
              ...
              {permissions.map((key) => (
                <label key={key}>
                  <input type="checkbox" name={key} defaultChecked={key !== 'accounts'} />
              ...
              {organizations.map((item) => (
                <label key={item.id}>
                  <input
                    ...
                    defaultChecked={
                      item.id ===
                      (user.organizationIds.includes('org_default')
                        ? 'org_default'
        ...
        <section className="account-list">
          ...
          {accounts.map((account) => (
            <article key={account.id}>
                ...
                {permissions.map((key) => (
                  <label key={key}>
                    <input
                      ...
                      checked={account.isSuperAdmin || account.permissions.includes(key)}
                      onChange={(event) =>
                        update(account, {
              ...
              <div className="organization-switches">
                {organizations.map((item) => (
                  <label key={item.id}>
                    <input
                      type="checkbox"
                      disabled={account.isSuperAdmin}
                      checked={account.isSuperAdmin || account.organizationIds.includes(item.id)}
                      onChange={(event) =>
                        toggleOrganization(account, item.id, event.target.checked)
                      }
                    />
                    {item.name}
                  </label>
              ...
              <button
                className={account.active ? 'disable' : 'enable'}
                disabled={account.id === user.id || account.isSuperAdmin}
                onClick={() => update(account, {active: !account.active})}
              >
          ...
          <div className="organization-list">
            ...
            {organizations.map((item) => (
              <div key={item.id}>
                <b>{item.name}</b>
                <span>{item.id}</span>
                <button onClick={() => updateOrganization(item)}>修改</button>
                <button
                  className="delete"
                  disabled={item.id === 'org_default'}
