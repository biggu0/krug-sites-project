...
async function localDatabase():Promise<AuthDb>{
  ...
  localDbPromise=(async()=>{
    ...
    const filePath=process.env.AUTH_DB_PATH??'/app/data/auth-db.json';
      ...
      return{
        ...
        async run(){
          ...
          if(normalized.startsWith('INSERT INTO USERS')){
            ...
            if(user){user.permissions=String(values[0]);user.active=values[1]?1:0;}
          }else if(normalized.startsWith('UPDATE USERS SET PASSWORD_HASH=')){
            const id=Number(values[2]),user=state.users.find(item=>item.id===id);
            if(user){user.password_hash=String(values[0]);user.salt=String(values[1]);}
...
export function sessionCookie(token:string,maxAge=60*60*12){const secure=process.env.AUTH_COOKIE_SECURE==='true'||(process.env.AUTH_COOKIE_SECURE!=='false'&&process.env.NODE_ENV==='production');return`jht_session=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Strict; Max-Age=${maxAge}${secure?'; Secure':''}`;}
