...
type TemplateDb = {
  prepare: (sql: string) => {
    bind: (...values: unknown[]) => TemplateDbStatement;
    run: () => Promise<unknown>;
    first: <T = unknown>() => Promise<T | undefined>;
    all: <T = unknown>() => Promise<{results: T[]}>;
  };
};
type TemplateDbStatement = {
  bind: (...values: unknown[]) => TemplateDbStatement;
  run: () => Promise<unknown>;
  first: <T = unknown>() => Promise<T | undefined>;
  all: <T = unknown>() => Promise<{results: T[]}>;
...
export function templateStorageUnavailableHint() {
  if (templateStorageProvider() === 'local')
    return '当前模板存储是 local。本地 Node/Docker 可以使用 local；wrangler/Cloudflare Worker 运行时不能写 node:fs，请改用 TEMPLATE_STORAGE_PROVIDER=cos。';
  if (templateStorageProvider() === 'database')
    return '当前模板存储是本地开发数据库；远程部署检测到已有 COS 配置后会自动继续使用 COS。';
...
export function templateStorageProvider() {
  ...
  const hasCosConfig = Boolean(
    (process.env.TENCENT_COS_SECRET_ID ?? process.env.COS_SECRET_ID) &&
    (process.env.TENCENT_COS_SECRET_KEY ?? process.env.COS_SECRET_KEY) &&
    (process.env.TENCENT_COS_REGION ?? process.env.COS_REGION) &&
    (process.env.TENCENT_COS_BUCKET ?? process.env.COS_BUCKET)
  );
...
function objectPrefix() {
  ...
  const segments = [projectPrefix, envPrefix, base]
...
async function templateDb(): Promise<TemplateDb> {
  ...
  await db
    .prepare(
      'CREATE TABLE IF NOT EXISTS template_objects (object_key TEXT PRIMARY KEY, content_type TEXT NOT NULL, body BLOB NOT NULL, updated_at INTEGER NOT NULL)'
    )
    .run();
  await db
    .prepare('CREATE INDEX IF NOT EXISTS templates_updated_idx ON templates(updated_at)')
    .run();
  await db
    .prepare(
...
function rowToTemplate(row: TemplateRow): TemplateMetadata {
  return {
    ...
    hasCover:
      row.has_cover === null || row.has_cover === undefined ? undefined : Boolean(row.has_cover),
    pageCount: row.page_count ?? undefined,
    pageMode: row.page_mode ?? undefined,
    duplex: row.duplex === null || row.duplex === undefined ? undefined : Boolean(row.duplex),
    rotateCover:
      row.rotate_cover === null || row.rotate_cover === undefined
        ? undefined
        : Boolean(row.rotate_cover),
    rotateInner:
      row.rotate_inner === null || row.rotate_inner === undefined
        ? undefined
        : Boolean(row.rotate_inner),
...
function cleanRegions(value: unknown) {
  ...
  for (const key of ['cover', 'inner'] as const) {
    ...
    if (
      ...
      throw new Error(`${key === 'cover' ? '封面' : '内页'}定制区域至少需要3个坐标点`);
...
async function validatePdfFile(file: File, kind: 'template' | 'foreground') {
  ...
  if (file.size > maxTemplateBytes)
    throw new Error(`PDF 文件不能超过 ${Math.round(maxTemplateBytes / 1024 / 1024)}MB`);
  if (!/\.pdf$/i.test(file.name)) throw new Error('模板文件扩展名必须是 .pdf');
  if (file.type && file.type !== 'application/pdf')
    throw new Error('模板文件 MIME 类型必须是 application/pdf');
  const pageCount = await pdfPageCount(file);
  if (kind === 'template' && ![12, 13, 24, 25].includes(pageCount))
...
function cleanMetadata(metadata: Partial<TemplateMetadata>, actualPageCount: number) {
  ...
  if (pageCount !== actualPageCount)
    throw new Error(`模板参数页数 ${pageCount} 与 PDF 实际页数 ${actualPageCount} 不一致`);
  if (![12, 13, 24, 25].includes(pageCount)) throw new Error(`模板页数不受支持：${pageCount}`);
  const duplex = booleanValue(metadata.duplex) ?? [24, 25].includes(pageCount),
    pageMode = pageModeValue(metadata.pageMode) ?? (duplex ? 'odd' : 'all');
...
function cosConfig(): CosConfig {
  ...
  if (!secretId || !secretKey || !region || !bucket)
    throw new Error(
      'COS 配置不完整，请检查 TENCENT_COS_SECRET_ID、TENCENT_COS_SECRET_KEY、TENCENT_COS_REGION、TENCENT_COS_BUCKET'
    );
...
async function cosAuthorization(
  config: CosConfig,
  input: {method: string; path: string; query?: Record<string, string>; host: string}
) {
...
async function cosFetch(
  ...
  const config = cosConfig(),
    client = await cosClient(config),
    key = objectKey(relativeKey),
    pathName = encodeCosPath(key),
    signPath = cosSignPath(key);
  const queryString = Object.entries(init.query ?? {})
    .map(([key, value]) => `${encodeQueryValue(key)}=${encodeQueryValue(value)}`)
    .join('&');
  const url = `${client.origin}${pathName}${queryString ? `?${queryString}` : ''}`;
  const headers = new Headers({
    Authorization: await cosAuthorization(config, {
      method: init.method,
      path: signPath,
      query: init.query,
      host: client.host
...
async function cosDeletePrefix(relativePrefix: string) {
  ...
  await Promise.all(
    keys.map((key) =>
      cosFetch(key.replace(objectPrefix(), ''), {method: 'DELETE'}).catch(() => undefined)
    )
  );
}
...
function storageError(error: unknown, context: string) {
  ...
  if (
    /operation not permitted|AccessDenied|403/i.test(`${code} ${message} ${value.statusCode ?? ''}`)
  ) {
    return new Error(
      `${context}。COS 密钥或存储桶策略没有权限，请确认允许 PutObject/GetObject/DeleteObject，并确认路径前缀在允许范围内。原始错误：${message}`
    );
  }
...
async function writeObject(relativeKey: string, file: File) {
  ...
  if (provider === 'database') {
    ...
    await db
      .prepare(
        'INSERT OR REPLACE INTO template_objects(object_key,content_type,body,updated_at) VALUES(?,?,?,?)'
      )
      .bind(relativeKey, file.type || 'application/pdf', body.toString('base64'), Date.now())
      .run();
    return;
  }
...
async function readObject(relativeKey: string) {
  ...
  if (provider === 'database') {
    const db = await templateDb(),
      row = await db
        .prepare('SELECT body FROM template_objects WHERE object_key=?')
        .bind(relativeKey)
        .first<{body: string | ArrayBuffer | Uint8Array | number[]}>();
    ...
    const bytes =
      typeof row.body === 'string'
        ...
        : row.body instanceof ArrayBuffer
          ...
          : ArrayBuffer.isView(row.body)
            ? Buffer.from(row.body.buffer, row.body.byteOffset, row.body.byteLength)
...
async function deleteTemplateObjects(template: TemplateMetadata) {
  ...
  if (provider === 'database') {
    const db = await templateDb();
    await db
      .prepare('DELETE FROM template_objects WHERE object_key LIKE ?')
      .bind(`${template.id}/%`)
      .run();
    return;
  }
...
export async function listTemplates() {
  const db = await templateDb(),
    rows = await db.prepare('SELECT * FROM templates ORDER BY updated_at DESC').all<TemplateRow>();
...
export async function listTemplatesForOrganization(organizationId: string) {
  const db = await templateDb(),
    rows = await db
      .prepare('SELECT * FROM templates WHERE organization_id=? ORDER BY updated_at DESC')
      .bind(organizationId)
      .all<TemplateRow>();
  return rows.results.map(rowToTemplate);
}
...
export async function getTemplate(id: string) {
  const db = await templateDb(),
    row = await db.prepare('SELECT * FROM templates WHERE id=?').bind(id).first<TemplateRow>();
  return row ? rowToTemplate(row) : undefined;
}

export async function readTemplateFile(
  id: string,
  kind: 'file' | 'foreground',
  user?: SessionUser
) {
...
export async function createTemplate(input: {
  ...
  const actualPageCount = await validatePdfFile(input.file, 'template'),
    metadata = cleanMetadata(input.metadata ?? {}, actualPageCount);
  const now = Date.now(),
    id = newTemplateId(),
    organizationId = input.organizationId,
    name = cleanName(input.name || input.file.name.replace(/\.pdf$/i, '')),
    normalized = normalizedName(name),
    fileName = pdfFileName(input.file.name || name),
    objectKey = `${id}/${fileName}`,
    db = await templateDb();
  if (
    await db
      .prepare('SELECT id FROM templates WHERE organization_id=? AND normalized_name=? AND id<>?')
      .bind(organizationId, normalized, id)
      .first()
  )
    throw new Error(`模板名称“${name.replace(/\.pdf$/i, '')}”已存在，请使用其他名称`);
  ...
  if (input.foregroundFile) {
    ...
    if (foregroundPageCount !== actualPageCount)
      throw new Error(`前景保护层页数 ${foregroundPageCount} 与背景模板 ${actualPageCount} 不一致`);
  ...
  try {
    await db
      .prepare(
        'INSERT INTO templates(id,organization_id,normalized_name,name,file_name,object_key,foreground_file_name,foreground_object_key,regions,has_cover,page_count,page_mode,duplex,rotate_cover,rotate_inner,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)'
      )
      .bind(
        id,
        organizationId,
        ...
        fileName,
        objectKey,
        dbValue(foregroundFileName),
        dbValue(foregroundObjectKey),
        dbValue(metadata.regions ? JSON.stringify(metadata.regions) : undefined),
        metadata.hasCover ? 1 : 0,
        metadata.pageCount,
        metadata.pageMode,
        metadata.duplex ? 1 : 0,
        metadata.rotateCover ? 1 : 0,
        metadata.rotateInner ? 1 : 0,
        now,
        now
      )
      .run();
...
export async function updateTemplate(
  id: string,
  patch: Partial<TemplateMetadata>,
  user?: SessionUser
) {
  const db = await templateDb(),
  ...
  if (!template) throw new Error('模板不存在');
  if (user) assertTemplateAccess(template, user);
  const name = patch.name === undefined ? template.name : cleanName(patch.name),
    normalized = normalizedName(name);
  if (
    await db
      .prepare('SELECT id FROM templates WHERE organization_id=? AND normalized_name=? AND id<>?')
      .bind(template.organizationId, normalized, id)
      .first()
  )
    throw new Error(`模板名称“${name.replace(/\.pdf$/i, '')}”已存在，请使用其他名称`);
  const pageCount = patch.pageCount === undefined ? template.pageCount : Number(patch.pageCount);
  if (!pageCount || ![12, 13, 24, 25].includes(pageCount))
    throw new Error(`模板页数不受支持：${pageCount}`);
  ...
  await db
    .prepare(
      'UPDATE templates SET normalized_name=?, name=?, regions=?, has_cover=?, page_count=?, page_mode=?, duplex=?, rotate_cover=?, rotate_inner=?, foreground_file_name=?, foreground_object_key=?, updated_at=? WHERE id=?'
    )
    .bind(
      normalized,
      name,
      ...
      metadata.rotateCover ? 1 : 0,
      metadata.rotateInner ? 1 : 0,
      dbValue(template.foregroundFileName),
      dbValue(template.foregroundObjectKey),
      updatedAt,
      id
    )
    .run();
...
export async function updateTemplateForeground(id: string, file: File, user?: SessionUser) {
  ...
  if (template.pageCount && pageCount !== template.pageCount)
    throw new Error(`前景保护层页数 ${pageCount} 与背景模板 ${template.pageCount} 不一致`);
  const foregroundFileName = pdfFileName(file.name || `${template.name}-foreground.pdf`),
    foregroundObjectKey = `${id}/${foregroundFileName}`;
  if (template.foregroundObjectKey && template.foregroundObjectKey !== foregroundObjectKey)
    await deleteObjectIfExists(template.foregroundObjectKey);
  ...
  await db
    .prepare(
      'UPDATE templates SET foreground_file_name=?, foreground_object_key=?, updated_at=? WHERE id=?'
    )
    .bind(foregroundFileName, foregroundObjectKey, template.updatedAt, id)
    .run();
  return template;
}
...
export function publicTemplate(template: TemplateMetadata) {
  return {
    ...
    foregroundUrl: template.foregroundObjectKey
      ? `/api/templates/${template.id}/foreground`
      : undefined
  };
}
